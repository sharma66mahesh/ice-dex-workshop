...
const Home = () => {
  ...
  useEffect(() => {
    async function x() {
      try {
        ...
        setAvailableReserveICZ(
          await fetchBalance(provider, exchangeContract.address)
        );
      } catch (err) {
        console.error(err);
  ...
  useEffect(() => {
    ...
    async function x() {
      ...
      setTokenBalance(
        await fetchBalance(provider, walletAddress, tokenContract)
      );
    }
    x();
  }, [updateData]);
  ...
  const handleSwap = async (values) => {
    ...
    try {
      let tx = null;
      if (
        values.from === SUPPORTED_TOKENS.MYTOKEN[0] &&
        values.to === SUPPORTED_TOKENS.ICZ[0]
      ) {
        // swap MYTOKEN with ICZ tokens
        const res1 = await tokenContract.approve(
          exchangeContract.address,
          ethers.utils.parseEther(values.amount)
        );
        await res1.wait();
        alert(res1.hash);
        tx = await exchangeContract.swapTokenForIcz(
          ethers.utils.parseEther(values.amount)
        );
      } else {
        // swap ICZ tokens with MYTOKEN
        tx = await exchangeContract.swapIczForToken({
  ...
  const handleAddLiquidity = async (values) => {
    ...
    try {
      const res1 = await tokenContract.approve(
        exchangeContract.address,
        ethers.utils.parseEther(values.token)
      );
      await res1.wait();
      alert(res1.hash);
      const res2 = await exchangeContract.addLiquidity(
        ethers.utils.parseEther(values.token),
        { value: ethers.utils.parseEther(values.ICZ) }
      );
      await res2.wait();
      alert(res2.hash);
    } catch (err) {
  ...
  return (
    <>
      <div className='grid-container'>
        ...
        <div className='grid-item'>
          <ReserveAvailable
            token={availableReserveToken}
            icz={availableReserveICZ}
          />
        </div>
        <div className='grid-item'>
          <SwapForm
